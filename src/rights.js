...
function expandRole(roleName, roleDefinitions, visited = new Set()) {
  ...
  inheritedRoles.forEach((inheritedRole) => {
    expandRole(inheritedRole, roleDefinitions, visited).forEach(
      (permission) => {
...
function resolveAuthorization(memberDns, config) {
  ...
  const isMember =
    allowedGroupDn || allowedGroupName
      ? isAllowedByGroup
  ...
  Array.from(matchedRoles).forEach((roleName) => {
    expandRole(roleName, roleDefinitions).forEach((permission) =>
      permissions.add(permission),
    );
  });
  ...
  const roleLabels = roles.map(
    (roleName) => roleDefinitions[roleName]?.label || roleName,
  );
...
function hasAnyPermission(user, permissions) {
  return asArray(permissions).some((permission) =>
...
function canViewCase(user, damageCase) {
  ...
  return hasPermission(user, "view_own_cases") && isOwner(user, damageCase);
}
...
function canEditCase(user, damageCase) {
  ...
  return hasPermission(user, "edit_own_cases") && isOwner(user, damageCase);
}
