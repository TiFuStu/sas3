...
async function getNextTechnicalId(pool, username, now) {
  ...
  const result = await pool
    ...
    .query(
      "SELECT COUNT(*) AS recordCount FROM dbo.DATSCHADENSFAELLE WHERE SFAID LIKE @likePattern",
    );
...
async function getNextCaseNumber(pool, now) {
  ...
  const result = await pool
    ...
    .input("likePattern", sql.NVarChar, likePattern)
    .query(
      "SELECT MAX(SFANUMMER) AS maxNumber FROM dbo.DATSCHADENSFAELLE WHERE SFANUMMER LIKE @likePattern",
    );

  const maxNumber = result.recordset[0]?.maxNumber;
  const currentSequence = maxNumber
    ? Number.parseInt(String(maxNumber).slice(-4), 10)
    : 0;
...
async function upsertUser(user) {
  ...
  await pool
    ...
    .input("displayName", sql.NVarChar, displayName).query(`
...
async function createDamageCase(damageCase) {
  ...
  await pool
    ...
    .input(
      "openClaimAmount",
      sql.Decimal(12, 2),
      toDecimal(damageCase.openClaimAmount),
    )
    ...
    .input("createdBy", sql.NVarChar, damageCase.createdBy).query(`
...
async function listDamageCases(options = {}) {
  ...
  const result = await pool
    ...
    .input("ownerUsername", sql.NVarChar, options.ownerUsername || "").query(`
SELECT *
...
async function getDamageCaseById(damageCaseId) {
  ...
  const result = await pool
    ...
    .query(
      "SELECT TOP 1 * FROM dbo.DATSCHADENSFAELLE WHERE SFAID = @damageCaseId",
    );
