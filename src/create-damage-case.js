...
async function apiFetch(url, options = {}) {
    ...
    if (!response.ok) {
        const errorMessage = typeof body === "string" ? body : body.error || "Unbekannter Fehler";
...
function populateForm(damageCase) {
    ...
    elements.openClaimAmount.value = Number(damageCase.openClaimAmount || 0).toFixed(2);
...
async function loadCaseList() {
    ...
    const data = await apiFetch(`/api/damage-cases?scope=${encodeURIComponent(selectedScope)}`);
    if (!data) {
        return;
    }
...
async function loadCase(caseId) {
    const damageCase = await apiFetch(`/api/damage-cases/${encodeURIComponent(caseId)}`);
    if (!damageCase) {
        return;
    }
...
async function saveCase() {
    ...
    const url = caseId ? `/api/damage-cases/${encodeURIComponent(caseId)}` : "/api/damage-cases";
