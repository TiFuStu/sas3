...
function requireAnyPermission(permissions) {
    return (req, res, next) => {
        ...
        res.status(403).json({ error: "Zugriff verweigert", requiredPermissions: permissions });
    };
}
...
async function fetchDirectoryUser(username) {
    ...
    try {
        ...
        const results = await ldapSearch(serviceClient, config.ldap.baseDN, {
            scope: "sub",
            filter,
            attributes: ["dn", "memberOf", "mail", "givenName", "sn", "displayName", "cn", "sAMAccountName"]
        });
...
function sanitizeDamageCaseInput(body) {
    return {
        ...
        subject: normalizeString(body.subject) || normalizeString(body.responsibleParty) || "Schadensfall",
        ...
        costsComplete: body.costsComplete === true || body.costsComplete === "true" || body.costsComplete === "on" || body.costsComplete === 1 || body.costsComplete === "1",
...
app.get("/create-damage-case", requireLogin, requireAnyPermission(["create_case", "view_own_cases", "view_all_cases"]), (_req, res) => {
...
app.get("/api/users", requireLogin, requirePermission("manage_users"), async (_req, res) => {
    try {
        const users = await db.getUsers();
        const enrichedUsers = await Promise.all(users.map(async (user) => {
            const username = normalizeString(user.BENADUID || user.BENSASXUSER || user.BENID);

            try {
                const directoryUser = username ? await fetchDirectoryUser(username) : null;
                const authorization = rights.resolveAuthorization(directoryUser?.memberOf, config);
                return {
                    ...user,
...
app.get("/api/damage-cases", requireLogin, requireAnyPermission(["view_own_cases", "view_all_cases"]), async (req, res) => {
    try {
        const requestedScope = req.query.scope === "all" ? "all" : "own";
        const includeAll = requestedScope === "all" && rights.hasPermission(req.session.user, "view_all_cases");
        ...
        const items = damageCases
            .filter((damageCase) => rights.canViewCase(req.session.user, damageCase))
...
app.get("/api/damage-cases/:id", requireLogin, requireAnyPermission(["view_own_cases", "view_all_cases"]), async (req, res) => {
    try {
        ...
        if (!rights.canViewCase(req.session.user, damageCase)) {
            res.status(403).json({ error: "Keine Berechtigung fuer diesen Schadensfall" });
            return;
        }
...
app.post("/api/damage-cases", requireLogin, requirePermission("create_case"), async (req, res) => {
    try {
        const payload = sanitizeDamageCaseInput(req.body);
        payload.createdBy = req.session.user.username;
        payload.assignedTo = payload.assignedTo || req.session.user.displayName || req.session.user.username;
...
app.put("/api/damage-cases/:id", requireLogin, requireAnyPermission(["edit_own_cases", "edit_all_cases"]), async (req, res) => {
    try {
        ...
        if (!rights.canEditCase(req.session.user, existingDamageCase)) {
            res.status(403).json({ error: "Keine Bearbeitungsrechte fuer diesen Schadensfall" });
            return;
        }
        ...
        const updatedDamageCase = await db.updateDamageCase(req.params.id, payload);
        ...
        res.status(500).json({ error: "Fehler beim Aktualisieren des Schadensfalls" });
    }
});
...
async function connectDatabaseWithRetry() {
    ...
    try {
        ...
        console.error(`Datenbank nicht erreichbar (${error.code || "DB_ERROR"}). Neuer Versuch in ${retryDelayMs / 1000}s.`);
